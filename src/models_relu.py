...
from .config_relu import LATENT_CH, LATENT_SCALE, COND_DIM, DEVICE, CLIP_MODEL_PATH, GROUPNORM_GROUPS
...
class ResBlock3D(nn.Module):
    ...
    def __init__(self, ch):
        ...
        self.net = nn.Sequential(
            nn.GroupNorm(GROUPNORM_GROUPS, ch, eps=1e-6), nn.ReLU(),
...
class Encoder3D(nn.Module):
    ...
    def __init__(self, in_ch=1, ch_mult=(64, 128, 128), latent_ch=LATENT_CH, scale=LATENT_SCALE):
        ...
        # Final: ResBlock + GroupNorm + ReLU + 1×1 conv to latent mean/logvar
        ch = ch_mult[-1]
        layers += [ResBlock3D(ch), nn.GroupNorm(GROUPNORM_GROUPS, ch, eps=1e-6), nn.ReLU(),
...
class Decoder3D(nn.Module):
    ...
    def __init__(self, latent_ch=LATENT_CH, ch_mult=(64, 128, 128), out_ch=1, scale=LATENT_SCALE):
        ...
        layers += [nn.GroupNorm(GROUPNORM_GROUPS, ch_final, eps=1e-6), nn.ReLU(),
...
class UNetBlock3D(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, t_dim, context_dim=COND_DIM, n_transformer=3,
                 downsample_ch=None, upsample_ch=None):
        super().__init__()
        ...
        self.upsample   = nn.ConvTranspose3d(out_ch, upsample_ch, 4, stride=2, padding=1) \
                          if upsample_ch is not None else None

    def forward(self, x, t_emb, ctx):
        # First ResBlock
        h  = self.conv1(F.relu(self.norm1(x)))
        h  = h + self.t_proj1(F.relu(t_emb))[:, :, None, None, None]
        h  = self.conv2(F.relu(self.norm2(h))) + self.skip1(x)
        for blk in self.trans1:
            h = blk(h, ctx)
        # Second ResBlock
        r  = self.conv3(F.relu(self.norm3(h)))
        r  = r + self.t_proj2(F.relu(t_emb))[:, :, None, None, None]
        h  = h + self.conv4(F.relu(self.norm4(r)))
...
class DenoisingUNet3D(nn.Module):
    ...
    def __init__(self, in_ch=LATENT_CH * 2, ch_list=(256, 512, 768),
        ...
        # Decoder — upsample operations inside each block
        self.dec3  = UNetBlock3D(c3 * 2, c3, t_dim, context_dim, upsample_ch=c2)
        self.dec2  = UNetBlock3D(c2 * 2, c2, t_dim, context_dim, upsample_ch=c1)
        self.dec1  = UNetBlock3D(c1 * 2, c1, t_dim, context_dim)

        self.out   = nn.Sequential(
            nn.GroupNorm(GROUPNORM_GROUPS, c1, eps=1e-6), nn.ReLU(), nn.Conv3d(c1, LATENT_CH, 1)
        )
    ...
    def forward(self, ht, t, ctx):
        ...
        # Decoder: unpack (skip, upsampled_out) from each block
        m = F.interpolate(m, size=e3.shape[2:], mode='trilinear', align_corners=False)
        _, up3 = self.dec3(torch.cat([m,   e3], dim=1), t_emb, ctx)
        up3 = F.interpolate(up3, size=e2.shape[2:], mode='trilinear', align_corners=False)
        _, up2 = self.dec2(torch.cat([up3, e2], dim=1), t_emb, ctx)
        up2 = F.interpolate(up2, size=e1.shape[2:], mode='trilinear', align_corners=False)
        _, d1  = self.dec1(torch.cat([up2, e1], dim=1), t_emb, ctx)
