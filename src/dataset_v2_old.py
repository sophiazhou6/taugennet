"""
dataset_v2.py – ADNI dataset loader supporting two conditioning modes.
...
class TauPETDataset(Dataset):
    ...
    def _load(self, path, mask=None):
        ...
        vol = (vol - orig_min) / (orig_max - orig_min + 1e-8)
...
def build_dataloaders(mode: str, base_dir=BASE_DIR, batch_size=BATCH_SIZE, seed=SEED,
    ...
    Split : 80% train / 10% val / 10% test (shuffled with seed before splitting).
    ...
    train_n = int(0.80 * n)
    val_n   = int(0.10 * n)
    # test_n absorbs rounding remainder (~10%)
