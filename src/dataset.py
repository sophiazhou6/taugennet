"""
dataset.py – ADNI dataset loader supporting two conditioning modes.
...
def build_dataloaders(mode: str, base_dir=BASE_DIR, batch_size=BATCH_SIZE,
    ...
    val_fraction : fraction of training subjects to hold out as validation
                   (carved from the 72% train split; test split is unchanged)
    ...
    # 72% train+val, 28% test — then split train+val into train / val
    trainval_end = int(0.72 * len(pet_paths))
    trainval_n   = trainval_end
